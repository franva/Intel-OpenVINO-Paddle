...
def postprocess(tracker, pred_dets, pred_embs, threshold):
    tensor_pred_dets = paddle.to_tensor(pred_dets)
    tensor_pred_embs = paddle.to_tensor(pred_embs)

    online_targets_dict = tracker.update(pred_dets, pred_embs)
    # online_targets_dict = tracker.update(tensor_pred_dets, tensor_pred_embs)
...
# from .fairmotlib import mot_jde_infer
...
tracker = JDETracker()
online_tlwhs, online_scores, online_ids = postprocess(tracker, pred_dets=result[output_names[0]], pred_embs=result[output_names[1]], threshold=0.5)
